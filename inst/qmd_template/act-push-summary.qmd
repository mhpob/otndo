...
execute:
  echo: false
title: "`r paste('ACT push summary:', params$project_name)`"
---
...
A total of `r format(nrow(qualified), big.mark = ',')` detections of `r uniqueN(qualified, by  = 'fieldnumber')` individuals have been matched via [ACT](`r paste0('https://matos.asascience.com/project/detail/', params$project_number)`), [FACT](https://secoora.org/fact/projects/), and/or [OTN](`r paste0('https://members.oceantrack.org/OTN/project?ccode=ACT.PROJ', params$project_number)`). The detections span across `r uniqueN(qualified, by = 'trackercode')` different projects consisting of `r length(unique(unlist(strsplit(pis$PI, ',\\s?'))))` principal investigators.
