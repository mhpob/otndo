---
params:
  matos_project: "My Project"
  qualified: "qual_dets"
  unqualified: "unqual_dets"
format: html
editor: visual
execute:
  echo: false
title: "`r paste('ACT push summary:', params$matos_project)`"
---

params needed:

-   project title (matos_project) (CHECK)
-   qualified location (CHECK)
...
# tar_read(deployment_gantt) +
#   labs(title = 'Temporal receiver coverage')
```
...
# new_matches <- qualified[datelastmodified > push_dates$previous]
```

Since the last data push in `r #format(push_dates$previous, '%B %Y')`, `r #format(nrow(qualified[datelastmodified > push_dates$previous]), big.mark = ',')` detections of `r #uniqueN(qualified[datelastmodified > push_dates$previous], by  = 'fieldnumber')` individuals have been updated or newly-matched.
...
#   unique(new_matches, by  = c('fieldnumber', 'station'))[, .(individuals = .N), by = 'station']
# ) |> 
#   DT(, station := sapply(station, .simpleCap))
# 
# setnames(station_summary, c('Station', 'Detections', 'Individuals'))
