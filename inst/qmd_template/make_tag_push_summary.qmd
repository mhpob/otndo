...
mt <- merge(mt, pis[, .(detectedby, PI, POC, emails)])
mt[, detectedby := ifelse(grepl('^PROJ', detectedby),
                           paste0('ACT.', detectedby),
                           ifelse(!grepl('^PROJ|^ACT|^FACT|^OTN', detectedby),
...
mt <- mt[, .(PI, POC, resource_full_name, network, code,
             detections, individuals, emails)]
setnames(mt, c('PIs', 'POC', 'Project name', 'Network', 'Project code',
               'Detections', 'Individuals', 'Contact info'))
...
ggplot() +
  geom_hline(yintercept = sort(unique(release$tagname))[
    seq(2, uniqueN(release$tagname), by = 2)], color = 'gray90') +
  
  geom_tile(data = matched_abacus,
            aes(x = day, y = tagname, fill = detectedby_plot)) +
  geom_tile(data = release, aes(x = day, y = tagname), width = 0.5)+
  scale_y_discrete(limits = sort(unique(release$tagname)),
...
station_summary <- merge(
  ...
  unique(matched, by = c('tagname', 'station'))[, .(individuals = .N,
                                                    long = mean(longitude),
                                                    lat = mean(latitude)),
                                                      by = 'station'] 
)
...
k <- last_record[release[, .(tagname, datecollected)], ,on='tagname']
setnames(k, 'datecollected', 'first_record')
k[, last_record := fifelse(is.na(last_record), first_record, last_record)]

k[, ':='(first_record = as.Date(first_record),
         last_record = as.Date(last_record))]

date_seq <- data.table(date = seq(
  min(k$first_record, na.rm =T),
  max(k$last_record, na.rm = T),
  by = 'day'
)
)
k[, sapply(date_seq,
           function(.){
             sum(between(., first_record, last_record) == T)
           })]
```
...
new_matches <- matched[datelastmodified > push_log$date[nrow(push_log) - 1]]
```

Since the last data push in `r format(push_log$date[nrow(push_log) - 1], '%B %Y')`, `r format(nrow(matched[datelastmodified > push_log$date[nrow(push_log) - 1]]), big.mark = ',')` detections of `r uniqueN(matched[datelastmodified > push_log$date[nrow(push_log) - 1]], by  = 'tagname')` individuals have been updated or newly-matched.
...
new_mt <- merge(
  ...
  unique(new_matches, by  = 'tagname')[, .(individuals = .N), by = 'detectedby']
)

new_mt <- merge(new_mt, pis[, .(detectedby, PI)])

new_mt[, detectedby := ifelse(grepl('^PROJ', detectedby),
...
station_summary <- merge(
  ...
  unique(new_matches, by  = c('tagname', 'station'))[, .(individuals = .N,
           long = mean(longitude),
