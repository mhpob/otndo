...
#| column: page
#| fig-cap: Table 1. Number of total detections and individual fish per OTN node project along with points of contact.figcap
...
#| fig-cap: Figure 3. Temporal coverage of receivers across time. Black bars indicate periods of deployments, while red bars denote download dates.
...
station_summary <- merge(
  ...
  unique(qualified, by = c('fieldnumber', 'station'))[, .(individuals = .N,
                                                          long = mean(longitude),
                                                          lat = mean(latitude)),
                                                      by = 'station'] 
)
...
Since `r format(since_date, '%B %Y')`, `r format(nrow(qualified[datelastmodified > since_date]), big.mark = ',')` detections of `r uniqueN(qualified[datelastmodified > since_date], by  = 'fieldnumber')` individuals have been updated or newly-matched.

```{r}
#| label: new-otn-match-table
#| column: page
#| fig-cap: Table 3. Total number of detections and number of individual fish heard per project receiver.
#| fig-cap-location: top
new_mt <- merge(
...
station_summary <- merge(
  ...
  unique(new_matches, by  = c('fieldnumber', 'station'))[, .(individuals = .N,
           long = mean(longitude),
