...
test_that("creates a named list with correct types", {
  ...
  expect_named(otn,
               c('otn_resources_metadata_points',
                 'project_metadata'))

  expect_s3_class(otn[[1]], 'data.frame')
  expect_s3_class(otn[[2]], 'data.frame')

  expect_named(otn$otn_resources_metadata_points,
               c('FID', 'collectioncode', 'report', 'resource_full_name', 'ocean',
                 'seriescode', 'status', 'collaborationtype', 'totalrecords',
                 'id', 'the_geom'))
  expect_named(otn$project_metadata,
               c("FID", "collectioncode", "id", "seriescode", "collaborationtype",
